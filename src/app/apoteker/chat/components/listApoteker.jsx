...
export default function ListApoteker({ apoteker }) {
    ...
    return (
        <div className="w-full shadow-md sm:w-2/6 sm:flex flex-col min-h-screen bg-white px-6 py-4">
            <Input
            ...
            {filteredApoteker?.map((apoteker) => (
                <Link key={apoteker?.id} href={`/apoteker/chat/${apoteker?.id}`} className="flex justify-between px-2 py-4">
                    <User
                        name={apoteker?.nama}
